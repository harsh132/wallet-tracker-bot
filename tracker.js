    ...
    .on("data", log => {
        ...
        db.findOne({ wallet: receiver }, function (err, doc) {
            if (!err && doc && doc.telegramId) {
                ...
                axios
                    .get(
                        `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage?chat_id=${doc.telegramId}&parse_mode=MarkdownV2&text=*Credit*%0A%0ASender%20%3A%20${sender}%0A%0A*%5C%2B${amountText}%20${tokenLookUp[token]}*`
                    )
        ...
        db.findOne({ wallet: sender }, function (err, doc) {
            if (!err && doc && doc.telegramId) {
                ...
                axios
                    .get(
                        `https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/sendMessage?chat_id=${doc.telegramId}&parse_mode=MarkdownV2&text=*Debit*%0A%0ASender%20%3A%20${sender}%0A%0A*%5C-${amountText}%20${tokenLookUp[token]}*`
                    )
                    .then(res => console.log("Result :", res.data))
                    .catch(err => console.log("Error :", err.data));
            }
